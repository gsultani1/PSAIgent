...
$global:HeartbeatTasksPath = "$global:ShelixHome\config\agent-tasks.json"
$global:HeartbeatLogPath = "$global:ShelixHome\logs\heartbeat.log"
...
function Register-AgentHeartbeat {
    ...
    $shelixHome = $global:ShelixHome

    # Generate bootstrap script
    $scriptsDir = "$global:ShelixHome\data"
    if (-not (Test-Path $scriptsDir)) { New-Item -ItemType Directory -Path $scriptsDir -Force | Out-Null }
    $scriptPath = Join-Path $scriptsDir "heartbeat_bootstrap.ps1"

    $scriptContent = @"
# Auto-generated by Shelix Register-AgentHeartbeat -- do not edit
# Interval: ${IntervalMinutes}m | Created: $(Get-Date -Format 'yyyy-MM-dd HH:mm')
`$ErrorActionPreference = 'Stop'
`$errorLog = '$shelixHome\logs\heartbeat_errors.log'
try {
    `$global:ShelixHome = '$shelixHome'
...
"@
    ...
    try {
        ...
        Register-ScheduledTask -TaskName 'Heartbeat' -TaskPath '\Shelix' `
            -Trigger $trigger -Action $action -Settings $settings `
            -Description "Shelix agent heartbeat (every ${IntervalMinutes}m)" `
            -Force | Out-Null

        Write-Host "[Heartbeat] Registered scheduled task: \Shelix\Heartbeat (every ${IntervalMinutes}m)" -ForegroundColor Green
...
function Unregister-AgentHeartbeat {
    ...
    #>
    try {
        Unregister-ScheduledTask -TaskName 'Heartbeat' -TaskPath '\Shelix' -Confirm:$false -ErrorAction Stop
        Write-Host "[Heartbeat] Unregistered scheduled task." -ForegroundColor Green

        # Clean up bootstrap script
        $scriptPath = "$global:ShelixHome\data\heartbeat_bootstrap.ps1"
...
function Get-HeartbeatStatus {
    ...
    try {
        $task = Get-ScheduledTask -TaskName 'Heartbeat' -TaskPath '\Shelix\' -ErrorAction Stop
        $taskInfo = Get-ScheduledTaskInfo -TaskName 'Heartbeat' -TaskPath '\Shelix\'
