...
$global:SecretPatterns = @(
    ...
    @{ Name = 'Generic Secret Assign'; Pattern = '(?i)(?<![A-Za-z])(password|secret|token|api_?key)\s*[:=]\s*["\x27]?(?!document\.|window\.|console\.|function\b|return\b|new\s|null\b|true\b|false\b|undefined\b|getElementById|querySelector|getAttribute|localStorage|sessionStorage|process\.|config\.|settings\.|env\.|os\.)[A-Za-z0-9/_\+\-]{12,}(?![A-Za-z0-9]*\.)' }
)
